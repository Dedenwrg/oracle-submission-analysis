...
### 1.3 How Did We Conduct the Analysis?
...
3. Define coverage modes:
   - **ALL-FX mode**: A row is considered valid if *all* FX and Autonity columns are non-null.  
   - **ANY-FX mode**: A row is considered valid if *at least one* FX or Autonity column is non-null.
4. Compute coverage metrics:
   - Per-timestamp coverage across all validators.  
   - Daily coverage (how many timestamps had full coverage vs. partial coverage).  
   - Per-validator missing rates.  
5. Check weekend vs. weekday differences by labeling days Monday=0 through Sunday=6.
...
def compute_coverage_metrics(
    ...
    """
    Compute coverage metrics for either ALL-FX or ANY-FX mode.
    """
    ...
    lf = lf.with_columns(
        (pl.col("submitted_fx_data") | pl.col("submitted_autonity_data")).alias(
            "submitted_any_data"
        )
    )
    ...
    lf_per_addr = (
        ...
        .agg(
            [
                pl.any("submitted_any_data").alias("any_submitted"),
    ...
    lf_timestamp_coverage = (
        ...
        .with_columns(
            [
                (pl.col("validators_seen") - pl.col("num_submitted_any")).alias(
                    "num_missing_any"
                )
            ]
        )
    ...
    lf_day_coverage = (
        ...
        .agg(
            [
                ...
                pl.max("validators_seen").alias("max_validators_in_day"),
            ]
        )
        .with_columns(
            [
...
def check_weekend_patterns(df_timestamp_coverage: pl.DataFrame, df_source: pl.DataFrame):
    ...
    df_weekday_cov = (
        ...
        .with_columns(
            [
                (pl.col("num_ts_full_coverage") / pl.col("num_timestamps")).alias(
                    "fraction_full_cov"
                ),
            ]
        )
...
### 1.4 What Have We Found?

Below we directly reference outcomes from the `results` dictionary obtained by executing the analysis. The results shown will automatically update when re-running this notebook with new or updated datasets.
...
**Validators with Significant Missing Rates (≥50%)**

```{python}
top_missing = results["ALL"]["df_validator_missing"].filter(
    pl.col("fraction_missing") >= 0.5
)

num_high_missing = top_missing.height
print(f"Number of validators missing ≥50% submissions: {num_high_missing}")

if num_high_missing > 0:
    for row in top_missing.iter_rows(named=True):
        addr = row["Validator Address"]
        frac_missing = row["fraction_missing"] * 100
        print(f"Validator {addr} missing rate: {frac_missing:.1f}%")
    print("These validators have significant reliability issues and require attention.")
else:
    print("No validators have a missing rate ≥50%; validator submissions are generally reliable.")
```

Validators listed here consistently miss submissions, signaling potential systemic issues.
