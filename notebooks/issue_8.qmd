...
### 8.3 How Did We Conduct the Analysis?
...
def analyze_weekend_market_closure_issues(
    ...
    df_agg_day = df_sub.group_by(["date_only","weekday_num"]).agg([
        pl.count("Validator Address").alias("num_submissions"),
...
results_issue8 = analyze_weekend_market_closure_issues(
...
```{python}
df_daily = results_issue8["df_submissions_day_agg"]
df_daily
```

```{python}
weekend_total = results_issue8["weekend_total_submissions"]
weekday_total = results_issue8["weekday_total_submissions"]
...
```{python}
yahoo_info = results_issue8["yahoo_weekend_info"]
