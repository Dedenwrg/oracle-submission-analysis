...
### 3.3 How Did We Conduct the Analysis?
...
def load_yahoo_finance_data(directory_pattern: str, pair_label: str) -> pl.DataFrame:
    ...
    lf = pl.concat(lf_list).sort("Datetime")
    lf = lf.select(
        [
...
def flag_suspicious_values(
    ...
    """
    Detect suspicious or out-of-range values in Oracle data.
    """
    new_cols = []
    ...
    for pair_label in fx_pairs:
        ...
        base_name = pair_label.replace(" Price", "")
        ...
        df_local = df_submissions.select(
            [
                pl.col("Timestamp_dt"),
                pl.col("Validator Address"),
                pl.col(decimal_col).alias("oracle_price_decimal"),
            ]
        ).with_columns([pl.col("Timestamp_dt").dt.truncate("1m").alias("ts_minute")])

        df_bench_trunc = df_bench.with_columns(
            [pl.col("timestamp_benchmark").dt.truncate("1m").alias("ts_minute_bench")]
        ).select(["ts_minute_bench", "benchmark_close"])

        df_joined = df_local.join(
            df_bench_trunc,
            left_on="ts_minute",
            ...
            [
                (
                    (pl.col("oracle_price_decimal") - pl.col("benchmark_close")).abs()
        ...
        df_flagged_fx = (
            ...
            .with_columns(
                [
                    # cond1
                    pl.when(
                        (pl.col("oracle_price_decimal").is_null())
                    ...
                    .alias("cond1"),
                    # cond2
                    pl.when(pl.col("oracle_price_decimal") >= threshold_big)
                    .then(pl.lit("Excessively large price; "))
                    .otherwise(pl.lit(""))
                    .alias("cond2"),
                    # cond3
                    pl.when(pl.col("rel_diff_from_bench") > percent_threshold)
            ...
            .with_columns(
                [
                    (pl.col("cond1") + pl.col("cond2") + pl.col("cond3")).alias(
                        "suspect_reason"
                    )
                ]
            )
    ...
    if required_cols.issubset(set(df_submissions.columns)):
        ...
        df_autonity = df_autonity.with_columns(
            [
                (
                    (
                        pl.col("ntn_usd_estimated") - pl.col("NTN-USD Price Decimal")
                    ).abs()
        ...
        df_autonity_suspect = df_autonity.with_columns(
            [
...
df_outliers = analyze_out_of_range_values(
    ...
    deviation_threshold=0.20,  # 20% threshold
)
```

---

### 3.4 What Have We Found?
