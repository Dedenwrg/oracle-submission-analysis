...
This document provides a straightforward summary of the key findings from an analysis of Autonity Oracle submissions data from December 2024.  The analysis examined multiple dimensions that affect the reliability, accuracy, and security of the on-chain price data supplied by validators.

## Overview of Issues Analysed

The investigation covered ten distinct issue areas:

1. **Missing or Null Submissions** – validators that failed to provide any non-null price values
2. **Irregular Submission Frequency** – deviations from the expected 30-second cadence
3. **Out-of-Range Values** – price outliers versus benchmark feeds, negative or zero values, and other sanity-check failures
4. **Stale / Lagging Data** – repeated identical prices indicating frozen data feeds
5. **Confidence-Value Anomalies** – hard-coded or otherwise non-informative confidence metrics
6. **Cross-Rate Inconsistency** – mathematical violations (e.g. `NTN-ATN × ATN-USD ≠ NTN-USD`)
7. **Timing / Synchronisation Issues** – clock skew and send-time offsets between validators
8. **Weekend Effects** – behavioural changes when major markets are closed
9. **Vendor Downtime** – correlated stoppages or "all-zero" placeholders indicating upstream outages
10. **Security / Malicious Behaviour** – manipulation attempts, censorship, or Sybil-style coordination

# Key Findings

## Missing or Null Submissions

- **Four validators** recorded a **100 % missing-data rate** for the entire month:
...
- The dataset contained **40 317 distinct submission timestamps**; on an **average day 91.5 %** of those timestamps were fully covered (i.e. ≥ 90 % of active validators supplied valid data for every pair).
- **Weekend vs weekday coverage:** **95.5 % vs 89.1 %**, indicating significantly better behaviour on weekends.
- Across all timestamps roughly **8.5 %** lacked complete data from the validator set.

## Irregular Submission Frequency

- Expected cadence: **1 submission every 30 s** (2 880 per day).
...
- The median stale-run length was **38 submissions**; **15 234 runs** persisted for > 1 h.
...
- Mean submission offsets (per validator, relative to the group median) ranged from **-3.0 s to +0.4 s**.
- The **maximum individual offset** observed was **15 s**; no validator breached the ±30 s alert threshold.
- The **median absolute offset** was **7.46 s**.  The most precise validator (0x7b06f608aB874E21f8FFC35D04B32bc03D8dCE1f) averaged **3.05 s**, while the least precise (0xbfDcAF35f52F9ef423ac8F2621F9eef8be6dEd17) averaged **7.70 s**.
...
- **8 476 timestamps** featured simultaneous extreme outliers (> 2 σ from the median) posted by at least three validators, concentrated around 18-19 December.
...
- **Bottom decile** exceeded *25 %* problematic submissions, emphasising the need for stronger incentives.
...
4. **Validator quality score** – weight **40 % uptime**, **30 % accuracy to benchmark**, **30 % consistency** and publish scores to incentivise improvements.
...
6. **Focused reviews** – prioritise investigation of the four validators exhibiting strong collusion / outlier patterns.

# Conclusion

While December 2024 showed measurable improvements in coverage and irregular-frequency metrics, the persistence of fully inactive validators and evidence of coordinated outlier submissions highlight ongoing risks.  Implementing the recommended validation checks, incentive mechanisms, and monitoring tools will substantially strengthen Oracle data quality and, by extension, the robustness of the Autonity ecosystem.
...
**Rating Methodology**  – identical thresholds and weighting to those used in previous monthly reports ensure comparability over time.
