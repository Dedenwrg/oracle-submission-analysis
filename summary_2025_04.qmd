---
title: "Oracle Submission Analysis - Summary of Key Findings (April 2025)"
format:
  html:
    toc: true
    toc-depth: 3
...
This document provides a straightforward summary of the key findings from an analysis of Autonity Oracle submissions data from April 2025. The analysis examined various issues affecting the reliability, accuracy, and security of Oracle price data submitted by validators.
...
- **Seven validators** had 100% missing-submission rates:
...
- **Average daily full-coverage (ALL-FX mode)**: **28.1%**
- Weekend full-coverage **24.2%** vs weekday **30.8%**
- Approximately **89,280 submission timestamps** were analyzed; **≈ 71.9%** of validator-slot combinations were missing at least one pair

## Irregular Submission Frequency

- Submission counts ranged from **0 to 2,880 per day** (expected 2,880)
- **10 validators** maintained ≥ 99% cadence for the whole month
- **9 validators** exhibited gaps exceeding 2h
- **≈ 11.2% of all submissions** occurred outside the expected 30-second interval
- Intervals within a **±5% tolerance band** around the 30-second target (0.475–0.525 min) were counted as **on-schedule**
- The median daily submission count across active validators was **2,698**
...
- **62,847 stale-data runs** were detected. Each run is defined as **≥ 30 identical consecutive submissions** for a given pair
- The longest run spanned **9,216 consecutive submissions** (~ 2.56h)
- **0 lagging windows** were observed in which price moved ≥ 5% within 60 min

## Confidence Value Anomalies

- **12 validators** supplied fixed confidence values (up from 10 in March)
- **195 validator-pair combinations** showed **near-zero correlation** (< 0.1) between confidence and price change
...
- Time drift between validators ranged from **0.6s** (best) to **18s** (worst)
- The **mean absolute offset** across the fleet was **8.1s**; no validator's mean exceeded **30s**
- **8 validators** consistently submitted **≥ 10s early**, while **6 validators** were **≥ 10s late** relative to the round median

## Weekend/Market-Closure Effects

- Weekend submissions totalled **1,987,234 rows** (**46.7%** of all data) – inconsistent with closed FX markets
- Weekend full-coverage was **6.6 pp lower** than weekdays (24.2% vs 30.8%)
...
- **8 isolated outage events** plus **one cluster of 45 validators** (15 Apr 14:30 UTC, ~ 75 min)
- **Cumulative downtime < 0.7%** – slight deterioration versus March

## Security/Malicious Behavior Indicators

- **3,512 validator-pair overlap events** (≥ 75% identical prices) were flagged
- **No multi-validator extreme-outlier events** were observed in April
...
1. **0x197B2c44b887c4aC01243BDE7E4b7E7b98A8d35A**  
   – 99.5% completeness • 0 flagged values • avg 0.15% deviation • dynamic confidence
2. **0xF9B38D02959379d43C764064dE201324d5e12931**  
   – 99.8% completeness • dynamic confidence • zero downtime
3. **0x23b4Be9536F93b8D550214912fD0e38417Ff7209**  
   – 99.7% completeness • avg 0.19% benchmark deviation • robust timing
...
4. **0x94470A842Ea4f44e668EB9C2AB81367b6Ce01772** – longest stale-run (9,216)
5. **0x3597d2D42f8Fbbc82E8b1046048773aD6DDB717E** – multiple stale runs > 6,100; weekend ratio 59%

## Validators with Coordinated Behavior

**Cluster 1** (NTN-ATN, 16 validators) – ≥ 18k identical overlaps  
**Cluster 2** (ATN-USD, 9 validators) – ≥ 91k identical overlaps  
**Cluster 3** (AUD-USD, 3 validators) – near-perfect mirroring

# Implications and Recommendations

## Data Quality Concerns

- Coverage gaps remain the dominant issue (71.9% of slots incomplete)
- Stale pricing is still prevalent (62,847 runs)
- Weekend pricing behavior conflicts with real-world FX market closures

## Validator Performance

- Top-10 validators: **≈ 1.9%** problematic submissions  
- Bottom-10 validators: **≈ 47.8%** problematic submissions
...
6. **Focused reviews** – prioritize investigation of the worst-performing validators and coordination clusters
...
| Issue Area | December 2024 | January 2025 | February 2025 | March 2025 | April 2025 | Trend |
|------------|:------------:|:------------:|:-------------:|:----------:|:----------:|:-----:|
...
- Missing/Null Submissions remained **Poor** with a seventh validator becoming fully inactive
- Irregular Submission Frequency deteriorated to **Fair** with 11.2% of submissions outside the expected interval
- Timing/Synchronization worsened to **Fair** with increased drift between validators
- Stale/Lagging Data remains **Excellent** despite increased stale run counts
- Overall rating remained **Good** with relatively stable performance across most metrics
